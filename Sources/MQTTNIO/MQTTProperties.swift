...
public struct MQTTProperties {
    ...
    init(_ properties: [PropertyId: PropertyValue]) {
    ...
    public mutating func addProperty(id: PropertyId, value: String) throws {
        guard id.type.type == .string else { throw MQTTError.invalidPropertyValue }
        properties[id] = .string(value)
    }
    
    public mutating func addProperty(id: PropertyId, value: (String, String)) throws {
        guard id.type.type == .stringPair else { throw MQTTError.invalidPropertyValue }
        properties[id] = .stringPair(value)
    }
    
    public mutating func addProperty(id: PropertyId, value: ByteBuffer) throws {
        guard id.type.type == .binaryData else { throw MQTTError.invalidPropertyValue }
    ...
    enum PropertyValue {
        ...
        static func read(from byteBuffer: inout ByteBuffer) throws -> (id: PropertyId, value: Self) {
            guard let idValue: UInt8 = byteBuffer.readInteger() else { throw MQTTError.badResponse }
            guard let id = PropertyId(rawValue: idValue) else { throw MQTTError.badResponse }
